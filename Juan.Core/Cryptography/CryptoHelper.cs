...
namespace Juan.Core
{
    ...
    public class CryptoHelper
    {
        ...
        {
            if (blowfishKey.Length % 8 != 0) throw new Exception("key的长度必须为8的倍数,并且不能大于56位");
            if (randomSource == null) randomSource = new RNGCryptoServiceProvider();
            SetupKey(HexToByte(blowfishKey));
        }
        ...
        {
            byte[] keyArray = UTF8Encoding.UTF8.GetBytes(key);
            byte[] toEncryptArray = UTF8Encoding.UTF8.GetBytes(toArray);
        ...
        {
            int paddedLen = (pt.Length % 8 == 0 ? pt.Length : pt.Length + 8 - (pt.Length % 8));
            //如果plainText长度不是8的倍数，则补全空格
            if (pt.Length % 8 != 0) pt = pt.PadRight(paddedLen, ' ');
        ...
        {
            return Encoding.ASCII.GetString(Decrypt_ECB(HexToByteDecyp(ct))).Replace(" ", "");
        }
