...
namespace Juan.Core
{
    public static class StringHelper
    {
        ...
        /// </summary>
        /// <param name="html"></param>
        /// <param name="replaceValue">标签替换字符</param>
        /// <returns></returns>
        ...
        public static string ClearHtmlFilter(this string html, params HtmlType[] objHtmlTypes)
        {
            if (!string.IsNullOrEmpty(html))
            {
                Regex regex;
                Match match;
                if (objHtmlTypes.Contains<HtmlType>(HtmlType.Tag) || (objHtmlTypes.Length == 0))
                {
                    html = Regex.Replace(html, "<[^>]*>", string.Empty, RegexOptions.IgnoreCase);
                }
                if (objHtmlTypes.Contains<HtmlType>(HtmlType.Script) || (objHtmlTypes.Length == 0))
                {
                    html = Regex.Replace(html, "</?script[^>]*>", string.Empty);
                    regex = new Regex("</?script[^>]*>", RegexOptions.IgnoreCase);
                    ...
                    for (match = regex.Match(html); match.Success; match = match.NextMatch())
                    {
                        html = html.Replace(match.Groups[0].ToString(), match.Groups[1].ToString() + "：");
                    }
                    regex = new Regex("on(mouse|exit|error|click|key)", RegexOptions.IgnoreCase);
                    for (match = regex.Match(html); match.Success; match = match.NextMatch())
                    {
                        html = html.Replace(match.Groups[0].ToString(), "<I>on" + match.Groups[1].ToString() + "</I>");
                    }
                    regex = new Regex("&#", RegexOptions.IgnoreCase);
                    for (match = regex.Match(html); match.Success; match = match.NextMatch())
                    {
                        html = html.Replace(match.Groups[0].ToString(), "<I>&#</I>");
                    }
                }
                if (objHtmlTypes.Contains<HtmlType>(HtmlType.Table) || (objHtmlTypes.Length == 0))
                {
                ...
                if (objHtmlTypes.Contains<HtmlType>(HtmlType.Class) || (objHtmlTypes.Length == 0))
                {
                    regex = new Regex("(<[^>]+) class=[^ |^>]*([^>]*>)", RegexOptions.IgnoreCase);
                    for (match = regex.Match(html); match.Success; match = match.NextMatch())
                    {
                        html = html.Replace(match.Groups[0].ToString(), match.Groups[1].ToString() + " " + match.Groups[2].ToString());
                    }
                }
                if (objHtmlTypes.Contains<HtmlType>(HtmlType.Style) || (objHtmlTypes.Length == 0))
                {
                ...
                if (objHtmlTypes.Contains<HtmlType>(HtmlType.Xml) || (objHtmlTypes.Length == 0))
                {
                    html = Regex.Replace(html, @"<\?xml[^>]*>", string.Empty);
                }
                if (objHtmlTypes.Contains<HtmlType>(HtmlType.Font) || (objHtmlTypes.Length == 0))
                {
        ...
        public static string ClearHtmlSymbol(this string html, string replacement = "")
        {
            ...
            {
                html = regex2.Replace(html, replacement);
            }
        ...
        public static string FilterKeyword(this string source, string Keyword, bool isRemove, string replaceString = "***")
        {
            if (!string.IsNullOrWhiteSpace(Keyword))
            {
                ...
                foreach (string str2 in Keyword.Split(separator, StringSplitOptions.RemoveEmptyEntries))
                {
                    char[] chArray2 = new char[] { '+' };
                    string[] strArray2 = str2.Split(chArray2, StringSplitOptions.RemoveEmptyEntries);
                    if (strArray2.Length == 1)
                    {
                        source = source.Replace(strArray2[0], isRemove ? "" : replaceString);
                        continue;
                    }
                    bool flag4 = true;
                    ...
                    if (flag4)
                    {
                        ...
                        {
                            source = source.Replace(str4, isRemove ? "" : replaceString);
                        }
                    }
                }
            }
        ...
        public static string FormatConcat(this string split, params object[] values)
        {
            ...
            {
                builder.Append(obj2.ToString() + split);
            }
        ...
        public static bool IsContainsKeyword(this string source, string Keyword)
        {
            ...
            char[] separator = new char[] { '|' };
            foreach (string str in Keyword.Split(separator, StringSplitOptions.RemoveEmptyEntries))
            {
                char[] chArray2 = new char[] { '+' };
                string[] strArray2 = str.ToLower().Split(chArray2, StringSplitOptions.RemoveEmptyEntries);
                if (strArray2.Length == 1)
                {
                    if (source.Contains(strArray2[0]))
                    {
                        flag = true;
                    }
                }
        ...
        public static bool IsContainsKeyword(this string source, string Keyword, out string ContainsKey)
        {
            ...
            ContainsKey = ContainsKey.Trim(trimChars);
            return flag;
        }
        ...
        public static long[] ToArrayInt64(this string value, bool isRemoveEmpty = true, char split = ',')
        {
            return value.ToList<long>(isRemoveEmpty, split).ToArray();
        }
        ...
        public static string ToChineseSpell(this string value, bool isAddSplit = true, char splitChar = ' ')
        {
            return value.ToPinyin(isAddSplit, splitChar);
        }

        public static string ToChineseSpellFirst(this string value, bool isUpper = true)
        {
            return value.ToPinyinFirst(isUpper);
        }
            ...
            if (Encoding.Default.GetByteCount(value) > length)
            {
                ...
                foreach (char ch in value)
                {
                    ...
                    if (num > length)
                    {
                        ...
                        {
                            value = value.Substring(0, num2 - tail.Length) + tail;
                            return value;
                        }
        ...
        public static DataTable ToDataTable(this string jsonValue)
        {
            ...
            return jsonValue.JsonDeserialize<List<Dictionary<string, object>>>(null).ToDataTable();
        }
        ...
        public static List<T> ToList<T>(this string value, bool isRemoveEmpty = true, char split = ',')
        {
            ...
            if (!value.IsNullOrWhiteSpace())
            {
                ...
                if (isRemoveEmpty)
                {
                    ...
                    strArray = value.Split(separator, StringSplitOptions.RemoveEmptyEntries);
                }
                else
                {
                    char[] chArray2 = new char[] { split };
                    strArray = value.Split(chArray2);
                }
        ...
        public static List<long> ToListInt64(this string value, bool isRemoveEmpty = true, char split = ',')
        {
            return value.ToList<long>(isRemoveEmpty, split);
        }
        ...
        public static string ToPinyin(this string value, bool isAddSplit = true, char splitChar = ' ')
        {
            ...
            char[] trimChars = new char[] { splitChar };
            return builder.ToString().TrimStart(trimChars);
        }

        public static string ToPinyinFirst(this string value, bool isUpper = true)
        {
            byte[] bytes = new byte[2];
            StringBuilder builder = new StringBuilder();
            int num = 0;
            ...
            for (int i = 0; i < chArray.Length; i++)
            {
                if (regex.IsMatch(chArray[i].ToString()))
                {
                    ...
                    else
                    {
                        ...
                        {
                            int num7 = Array.IndexOf<string>(referenceChinese, str);
                            builder.Append(isUpper ? pinYinArray[num7].ToUpper() : pinYinArray[num7]);
                        }
                    }
                }
                else
